...
- (IBAction)wechatpay:(id)sender {
    ...
    [CEFPayManager CEFServicePayWithEID:EID
                            ...
                            tradeNumber:@"DevTradeNumber001"
                                 amount:@"1"
                               callBack:^(CEFServicePayResult payResult, NSString *errorMessage,NSString *orderId) {
                                   
                                   NSLog(@"errCode = %zd,errStr = %@,orderId = %@",payResult, errorMessage,orderId);
...
-(void)queryOrder {
    [CEFPayManager CEFCheckingOrderWithOrderId:@"" order:^(NSDictionary *order) {
